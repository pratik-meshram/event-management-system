...
export default function UserCreateEvent() {
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      setUploading(true);
      const res = await API.post("/upload/event", fd, {
  ...
  return (
    <DashboardLayout role="user" userName={userName}>
      <button onClick={() => navigate("/user/my-events")}
      ...
      <div className="max-w-3xl">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            {isEdit ? "Update event details" : "Fill in the details — your event will be reviewed by admin before going live"}
          </p>
        </div>
        ...
        {!isEdit && (
          <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-100 rounded-2xl mb-6">
            ...
            <p className="text-amber-700 text-sm">
              After submission your event will be in <strong>pending</strong> status until an admin approves it.
            </p>
          </div>
        )}
              ...
              <div className="relative rounded-2xl overflow-hidden h-48 group">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3">
                  <button onClick={() => fileRef.current.click()}
                    className="bg-white text-slate-800 text-xs font-semibold px-4 py-2 rounded-xl">Change</button>
                  <button onClick={() => set("image", "")}
                    className="bg-red-500 text-white text-xs font-semibold px-4 py-2 rounded-xl">Remove</button>
                </div>
              </div>
            ) : (
              <button onClick={() => fileRef.current.click()} disabled={uploading}
                className="w-full h-40 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-3 hover:border-indigo-300 hover:bg-indigo-50/30 transition disabled:opacity-60">
                {uploading
                  ? <><Loader size={24} className="text-indigo-400 animate-spin" /><p className="text-slate-400 text-sm">Uploading...</p></>
                  : <><ImagePlus size={28} className="text-slate-300" /><p className="text-slate-400 text-sm">Click to upload banner</p><p className="text-slate-300 text-xs">JPG, PNG, WebP — max 5MB</p></>
                }
              </button>
            )}
          </div>
          ...
          <div className="flex gap-3">
            <button onClick={handleSubmit} disabled={loading || uploading}
              ...
              {loading
                ...
                : <><Save size={15} />{isEdit ? "Update Event" : "Submit Event"}</>
              }
            </button>
            <button onClick={() => navigate("/user/my-events")}
